...
def upload_image():
    ...
    image_path = os.path.join(app.config['UPLOAD_FOLDER'], image.filename)
    autograph_path = os.path.join(app.config['UPLOAD_FOLDER'], autograph.filename)
    autograph_transparent_path = os.path.join(app.config['UPLOAD_FOLDER'], 'transparent_' + autograph.filename)
    
    image.save(image_path)
    autograph.save(autograph_path)

    # Remove white background from the autograph
    remove_white_background(autograph_path, autograph_transparent_path)

    # Open the images
    base_image = Image.open(image_path)
    autograph_image = Image.open(autograph_transparent_path)

    # Resize autograph to fit within the base image
    autograph_image.thumbnail((base_image.size[0] // 2, base_image.size[1] // 2), Image.ANTIALIAS)
